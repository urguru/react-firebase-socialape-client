...
class Notifications extends Component{
    ...
    onMenuOpened=()=>{
        let unreadNotificationsIds=this.props.notifications.filter(not=>not.read==='false').map(not=>not.notificationId)
    ...
    render(){
        ...
        if(notifications && notifications.length>0){
            (notifications.filter(not=>not.read==="false").length>0)
                ? (
                    notificationsIcon = <Badge badgeContent={notifications.filter(not => not.read === "false").length} color="secondary">
                    <NotificationsIcon />
                </Badge>):(
        ...
        let notificationsMarkup=(notifications && notifications.length>0 )?(
            notifications.map(not=>{
                ...
                const iconColor=not.read==='false'?'secondary':'primary'
